...
## Se utilizan los datos reducidos mediante PCA
...
for (i in 1:10) {
  ...
  #Reducir datos mediante PCA
  pca <- prcomp(data.train)
  data.train <- as.data.frame(pca$x[,c(1:9)])
  
  pca <- predict(pca, data.test)
  data.test <- as.data.frame(pca[,c(1:9)])
...
write.csv(results, file = "benchmark_L_pca.out")
...
ggplot(melt(as.matrix(results)), aes(x = Var1, y = value)) + 
  ...
  ggtitle("Precisión obtenida para L usando PCA")