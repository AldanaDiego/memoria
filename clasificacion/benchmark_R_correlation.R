...
## Se utilizan los datos eliminando columnas con alta correlacion
...
write.csv(results, file = "benchmark_R_correlation.out")
...
ggplot(melt(as.matrix(results)), aes(x = Var1, y = value)) + 
  ...
  ggtitle("Precisión obtenida para R con datos sin correlaciones")